...
export type PublicationStrategy =
  (typeof PublicationStrategies)[keyof typeof PublicationStrategies];
 ...
 */
export class PublicationStrategyManager {
  private strategies: Map<string, PublicationStrategy> = new Map();
  ...
  setStrategy(collectionName: string, strategy: PublicationStrategy): void {
    if (!Object.values(PublicationStrategies).includes(strategy)) {
      throw new Error(
        `Invalid publication strategy for collection ${collectionName}`
      );
    }
  ...
  getStrategy(collectionName: string): PublicationStrategy {
    return (
      this.strategies.get(collectionName) || PublicationStrategies.SERVER_MERGE
    );
  }
