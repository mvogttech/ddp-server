...
class MessageParser {
    ...
    constructor(logger, metrics) {
        this.batchBuffer = []; // Buffer for batched messages
     ...
     */
    async parseMessage(data, isBinary) {
        try {
            ...
            throw new errors_1.DDPProtocolError('Invalid message format', {
                cause: error,
                isBinary,
            });
        }
    ...
    parseBinary(data) {
        try {
            ...
            throw new errors_1.DDPProtocolError('Failed to decode CBOR message', {
                cause: error,
            });
        }
    }
    ...
    parseText(data) {
        try {
            ...
            throw new errors_1.DDPProtocolError('Failed to parse JSON message', {
                cause: error,
            });
        }
    }
    ...
    validateMessage(message) {
        if (!message || typeof message !== 'object') {
            throw new errors_1.DDPProtocolError('Message must be an object');
        }
        if (!('msg' in message) || typeof message.msg !== 'string') {
            throw new errors_1.DDPProtocolError('Message missing or invalid "msg" field');
        }
        ...
        switch (message.msg) {
            case 'connect':
                if (typeof message.version !== 'string' ||
                    ...
                    throw new errors_1.DDPProtocolError('Invalid connect message: missing version or support');
                }
                break;
            case 'sub':
                if (typeof message.id !== 'string' ||
                    typeof message.name !== 'string') {
                    throw new errors_1.DDPProtocolError('Invalid sub message: missing id or name');
                }
                if ('params' in message && !Array.isArray(message.params)) {
                    throw new errors_1.DDPProtocolError('Invalid sub message: params must be an array');
                }
                break;
            case 'unsub':
                if (typeof message.id !== 'string') {
                    throw new errors_1.DDPProtocolError('Invalid unsub message: missing id');
                }
                break;
            case 'method':
                if (typeof message.id !== 'string' ||
                    typeof message.method !== 'string') {
                    throw new errors_1.DDPProtocolError('Invalid method message: missing id or method');
                }
                if ('params' in message && !Array.isArray(message.params)) {
                    throw new errors_1.DDPProtocolError('Invalid method message: params must be an array');
                }
                break;
            case 'ping':
            case 'pong':
                if ('id' in message && typeof message.id !== 'string') {
                    throw new errors_1.DDPProtocolError('Invalid ping/pong message: id must be a string');
                }
                break;
            default:
                throw new errors_1.DDPProtocolError(`Unsupported message type: ${message.msg}`);
        }
    }
    /**
     ...
     */
    bufferMessage(message, send, useBinary = true) {
        ...
        if (this.batchBuffer.length >= 10) {
            ...
            this.batchTimeout = setTimeout(() => this.flushBuffer(send, useBinary), 10);
        }
    }
    /**
    ...
    flushBuffer(send, useBinary) {
        ...
        try {
            ...
            const encoded = useBinary
                ? Buffer.from((0, cbor_x_1.encode)(messages))
                : Buffer.from(JSON.stringify(messages), 'utf-8');
            send(encoded);
            this.logger.debug(`Encoded and sent ${messages.length} batched message(s)`, { useBinary });
            ...
            throw new errors_1.DDPProtocolError('Failed to encode batched messages', {
                cause: error,
                useBinary,
            });
        }
     ...
     */
    encodeMessage(message, useBinary) {
        const encoded = useBinary
            ? Buffer.from((0, cbor_x_1.encode)(message))
